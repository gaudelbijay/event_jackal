...
from envs.jackal_real_envs import JackalRobotEnv, JackalRobotEvents

class MotionControlContinuous(JackalRobotEnv):
    def __init__(self, min_v=-1, max_v=2, min_w=-3.14, max_w=3.14, **kwargs):

        super().__init__(**kwargs)

        # now your control‐wrapper logic
        self.action_dim = 2
        ...
        self.action_space = Box(
            low=np.array([min_v, min_w]),
            high=np.array([max_v, max_w]),
            dtype=np.float32
        )
    ...
    def reset(self):
        ...
        return obs, {"initial position": pos}
...
class RealMotionControlContinuousEvents(MotionControlContinuous, JackalRobotEvents):
