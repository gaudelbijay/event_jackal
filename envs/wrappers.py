...
class ShapingRewardWrapper(gym.Wrapper):
    ...
    def step(self, action):
        obs, rew, done, turncated, info = self.env.step(action)
        position = self.env.gazebo_sim.get_model_state().pose.position
        rew += position.y - self.Y
        self.Y = position.y
        return obs, rew, done, turncated, info
...
class StackFrame(gym.Wrapper):
    ...
    def step(self, *args, **kwargs):
        obs, rew, done, turncated, info = self.env.step(*args, **kwargs)
        self.frames.append(obs)
        return np.stack(self.frames), rew, done, turncated, info