...
worlds_dir = "base_worlds/hybrid_base_worlds"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_object', type=int, default=5)
    parser.add_argument('--start_idx', type=int, default=130)
    ...
    i = 0
    cwd = os.getcwd()
    for world in glob.glob(os.path.join(worlds_dir, '*.world')):
        with open(world, 'r') as f:
            ss = f.read()
            part1 = ss.split("ACTORS")[0]
            part2 = ss.split("ACTORS")[1]
            f.close()
        mid = ""
        for j in range(np.random.randint(args.min_object, args.max_object)):
            angle, start, end = sample_waypoints(np.random.choice(DIRECTIONS), args.min_speed, args.max_speed)
            mid += make_moving_model(j, start[0], start[1], end[0], end[1], angle, end[2])
