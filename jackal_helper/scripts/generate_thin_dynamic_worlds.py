...
BASE_WORLD_PATH = "base_worlds/base_world_thin.world"
...
def sample_waypoints(direction, min_speed, max_speed, idx):
    ...
    time = distance / speed
    return "thin_%s_%d_%d" %(direction, int(speed * 100), idx), [(0, start_x, start_y), (time, end_x, end_y)]
...
def make_random_shape():
    ...
    if shape == 0: #cylinder
        modelShape = "<cylinder>\n\
                <radius>{}</radius>\n\
                <length>{}</length>\n\
                </cylinder>".format(np.random.uniform(.2, .5), np.random.uniform(.2, .5))
    elif shape == 1: #box
        modelShape = "<box>\n\
                <size>{} {} {}</size>\n\
                </box>".format(np.random.uniform(.2, .5), np.random.uniform(.2, .5), np.random.uniform(.2, .5))
    else: #sphere
        modelShape = "<sphere>\n\
                <radius>{}</radius>\n\
                </sphere>".format(np.random.uniform(.2, .5))
    return modelShape
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_idx', type=int, default=70)
    ...
    plugins = [f for f in os.listdir(plugins_build_dir) if f.endswith(".so") and f.startswith("libthin")]
    ...
    for i in range(args.n_worlds):
        ...
        for j in range(np.random.randint(args.min_object, args.max_object)):
            ...
            mid += make_moving_model(plugin, color, shape) #################### add color and shape
