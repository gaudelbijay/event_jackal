...
def make_random_shape():
    ...
    if shape == 0: #cylinder
        modelShape = "<cylinder>\n\
                <radius>{}</radius>\n\
                <length>{}</length>\n\
                </cylinder>".format(np.random.uniform(.08, .6), np.random.uniform(.08, .6))
    elif shape == 1: #box
        modelShape = "<box>\n\
                <size>{} {} {}</size>\n\
                </box>".format(np.random.uniform(.08, .6), np.random.uniform(.08, .6), np.random.uniform(.08, .6))
    else: #sphere
        modelShape = "<sphere>\n\
                <radius>{}</radius>\n\
                </sphere>".format(np.random.uniform(.08, .6))
    return modelShape
...
if __name__ == "__main__":
    ...
    parser.add_argument('--min_object', type=int, default=5) ########################
    parser.add_argument('--max_object', type=int, default=8) #######################
    ...
    plugins = [f for f in os.listdir(plugins_build_dir) if f.endswith(".so") and not f.startswith("libthin") and not f.startswith("libwide") and not f.startswith("libActor")]
