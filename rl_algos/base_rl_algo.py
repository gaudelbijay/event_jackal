...
class BaseRLAlgo:
    ...
    def scale_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        action = torch.clamp(action, -1.0, 1.0)  # Safety clamp
        # Scale from [-1, 1] to [low, high]
        action = self._action_scale * action + self._action_bias
        return action

    def unscale_action(self, action: torch.Tensor) -> torch.Tensor:
        """Scale action from actual action space to [-1, 1]"""
        # Scale from [low, high] to [-1, 1]
        action = (action - self._action_bias) / self._action_scale
        return torch.clamp(action, -1.0, 1.0)  # Safety clamp
    ...
    def save(self, dir, filename):
        # Unwrap actor from DataParallel if necessary and move parameters to CPU
        if hasattr(self.actor, "module"):
            state_dict = self.actor.module.state_dict()
        else:
            state_dict = self.actor.state_dict()
        # Ensure that all tensors in state_dict are on CPU for saving
    ...
    def load(self, dir, filename):
        ...
        # Load state into the underlying module if using DataParallel
        if hasattr(self.actor, "module"):
            self.actor.module.load_state_dict(state_dict)
        else:
            self.actor.load_state_dict(state_dict)
        # Recreate actor target if needed (and move to current device)
