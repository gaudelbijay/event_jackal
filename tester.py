...
def get_world_name(config, id):
    # We test each test world with two actors, so duplicate the list by a factor of two
    world_name = (config["container_config"]["test_worlds"] * 2)[id]
...
def main(args):
    ...
    num_trials = 10#config["container_config"]["num_trials"]
    env_config = config['env_config']
    world_name = get_world_name(config, args.id)
    test_object = "local"#config["container_config"]["test_object"]
